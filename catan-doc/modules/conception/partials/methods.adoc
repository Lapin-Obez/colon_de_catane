...
Appelle `player.removeDevelopmentCard(MONOPOLY)` pour le joueur actuel.
...
Appelle `player.removeDevelopmentCard(YEAR_OF_PLENTY)`.
...
Appelle `player.removeDevelopmentCard(ROAD_BUILDING)`.
...
|`void removeDevelopmentCard(Development type)`
a| Supprime de la main d'un joueur une carte Development de type `type`.
